...
- **Deployment** (`render.yaml`) runs the API and UI on Render (Frankfurt, free web services, HTTPS). Facts live in **Neon Postgres** (`DATABASE_URL`); Render's free database expires after 90 days and is no longer used. The API falls back to committed seed JSON when the database is unreachable. A scheduled GitHub Action (`.github/workflows/ingest.yml`) triggers `POST /refresh?run_ingest=true` daily to keep Neon current.
