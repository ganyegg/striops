...
This document is the single source of truth for what Helm is, how it works, what data it touches, how the 90-day pilot succeeds, why R180,000 is justified, where it sits in the City, and how to answer every hard question in the room.
...
**Helm is the strategic twin of Cape Town — the city, modelled, watched, and argued over by software that shows its work.**
...
For each metric series \(y_t\) (monthly points), Helm fits a simple linear trend:
...
- Executive brief is **cached in-process for 1 hour** (`HELM_BRIEF_TTL_SECONDS=3600`) to control cost and latency. Greeting still refreshes with time of day.
...
**Honesty note:** the seed series currently ends in **February 2026**. If today is later, Helm must still say “Data through February 2026” — never pretend the series is current. The pilot’s job is to replace that seed with live departmental extracts so `data_through` moves every month (target: daily for selected feeds).
...
- **Helm path:** [`backend/helm/ingestion/treasury.py`](../backend/helm/ingestion/treasury.py) with local cache under `datasets/cache/`.
...
- **Helm path:** [`backend/helm/ingestion/arcgis.py`](../backend/helm/ingestion/arcgis.py).
...
- **IT path:** City IT change request → read-only interface user or SFTP drop → Helm ingest job.
...
- **IT path:** Agreed extract schema (period, zone_id, nrw_pct, system_input_kl, losses_kl) → SFTP or API → Helm.
...
### 4.2 Where Helm sits — three postures

| Posture | Description | When to use |
|---|---|---|
| **A. City tenancy** | Helm deployed in CoCT Azure (or equivalent) subscription; City controls network, keys, logs | Preferred for production / sensitive extracts |
| **B. SA-region managed cloud** | Helm-operated VPC in Johannesburg/Cape Town region; City accesses via SSO + IP allowlist | Fastest pilot if City IT queue is slow |
...
- URL (pilot): HTTPS app, e.g. `https://helm.capetown.gov.za` or a City-approved subdomain.
...
| Data class | In Helm today? | Pilot? | Notes |
|---|---|---|---|
...
| Citizen names / account numbers / ID numbers | **No** | **No** | Out of scope forever for Helm brief |
...
Helm’s design constraint: **executive intelligence on aggregates**. The MOU must forbid bulk personal information. If a feed cannot be aggregated upstream, it does not enter Helm.
...
This is how Helm answers: *Who is doing something about this, by when?*
...
Each entry: what Helm surfaced → linked action → outcome → `value_zar` with basis:
...
| Week | Outcome | Owner |
|---|---|---|
| 1 | Pilot order + MOU signed; sponsor named (Exec Mayor’s office or nominee) | City + Helm |
| 1 | Extract schemas agreed for Finance + Water (CSV columns, cadence, SFTP) | City IT + departments + Helm |
| 2 | Helm deployed in agreed posture (B/C → path to A); SSO smoke test | Helm + City IT |
| 2–3 | Public feeds automated (Treasury refresh, DWS dams) | Helm |
| 3–4 | First Finance extract landing in Helm; underspend opportunities go live | Finance + Helm |
| 4 | First NRW / zone extract landing; Pulse periods move with real months | Water + Helm |
| 4 | Weekly demo to sponsor (non-negotiable cadence) | Helm |
...
9. **Analyst pairing** — one City analyst co-owns the brief so Helm is a force multiplier, not a threat.
...
| Helm Core (post-pilot) | R95,000 / month (~R1.14m / year) |
| Helm Command | R160,000 / month |
...
**Break-even:** if Helm-caused action recovers **~0.001 percentage points** of NRW for a year, the pilot has paid for itself on water alone (\(0.001 \times 185\text{m} = R185{,}000\)).
...
Yes — once. Helm produces it every morning from live systems, with every number able to defend itself, and it never forgets why a decision was made.
...
# Backend
cd backend && .venv/bin/uvicorn helm.api.main:app --host 127.0.0.1 --port 8000
...
Key env vars: `GEMINI_API_KEY`, `GEMINI_MODEL`, `HELM_BRIEF_TTL_SECONDS`, `HELM_MUNICIPALITY=CPT`, Postgres/Neo4j settings (optional).

Health: `GET /health` → `llm_provider`, `facts_backend`.

Tests: `cd backend && .venv/bin/python -m pytest tests/ -q`

Push (IGG workspace): remote is `https://github.com/ganyegg/helm.git` (folder may still be named `helm-ai` locally).
...
Full in-product glossary: `GET /glossary` and [`backend/helm/core/glossary.py`](../backend/helm/core/glossary.py).
...
- Helm code — [github.com/ganyegg/helm](https://github.com/ganyegg/helm)
- Pitch deck — [`docs/pitch/helm-pitch.html`](pitch/helm-pitch.html)
