...
export default function WinCard({ win }: { win: Initiative }) {
  return (
    <Link
      ...
      className="card-win group block overflow-hidden transition hover:border-helm-ocean/50 hover:shadow-glow"
    >
      {win.image_url ? (
        <div className="relative h-36 overflow-hidden">
          ...
          <span className="absolute bottom-3 left-4 pill bg-helm-ocean/90 text-white">
            {win.category}
          </span>
        </div>
      ) : null}
      <div className="p-5">
        ...
        <p className="mt-2 text-sm font-medium text-helm-sand/90">{win.headline}</p>
        ...
        {win.metrics?.[0] ? (
          <div className="mt-4 flex items-end justify-between border-t border-white/10 pt-3">
            <div>
              ...
              <p className="font-display text-xl font-semibold text-helm-good">{win.metrics[0].value}</p>
            </div>
