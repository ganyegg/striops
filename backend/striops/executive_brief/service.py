...
from striops.agents import SPECIALIST_AGENTS, AgentContext, ExecutiveAgent
from striops.core.cache import cache_get, cache_set
...
from striops.recommendation_engine import recommend
from striops.valuation import attach_valuations

log = get_logger("striops.executive_brief")
...
def _health_score(risks: list[Risk], opportunities: list[Opportunity]) -> int:
    from striops.health_score import compute_health_breakdown
...
def build_executive_brief(
    ...
    city = _MUNICIPALITY_NAMES.get(settings.striops_municipality, settings.striops_municipality)

    # The brief is expensive (a dozen LLM calls); serve a cached copy within
    # the TTL, refreshing only the time-of-day greeting.
    cache_key = f"brief:{settings.striops_municipality}"
    if settings.striops_brief_ttl_seconds > 0:
        cached = cache_get(cache_key, settings.striops_brief_ttl_seconds)
    ...
    risks, opportunities = attach_valuations(
        risks, opportunities, ctx.metric_series, settings.striops_municipality
    )
    from striops.demographics import attach_affected

    risks = attach_affected(risks, settings.striops_municipality)
