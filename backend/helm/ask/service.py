"""Ask Helm — grounded natural-language answers over retrieved facts."""
...
from helm.health_score import build_health_breakdown
from helm.persistence import get_repository
from helm.places import detect_places, place_related_evidence
...
class AskResponse(BaseModel):
    ...
    ai_role: str = (
        "Narrator over retrieved Helm facts — engines own the numbers; "
        "AI does not invent metrics. Missing data is stated as a gap, not filled in."
    )
    model: str
    dynamic_note: str = (
        "Helm surfaces whatever is in the facts store after each ingest/refresh. "
...
def _retrieve_context(
    ...
    citations: list[AskCitation] = [
        ...
        AskCitation(label="Ask Helm", href="/ask"),
    ]
    facts: list[str] = [
        ...
        for g in place.get("gaps", []):
            ...
            gaps.append(
                DataGap(
                    ...
                    blocker=g,
                    ask_prompt=place.get("ask_prompt") or f"What does Helm know about {place['name']}?",
                )
            )
    ...
    context = {
        ...
        "places": place_packs,
        "place_protocol": (
            "If places[] is non-empty, lead with a place briefing: what Helm can evidence "
    ...
    if ctx.get("places"):
        ...
        if "hospital" in q:
            lines.append(
                ...
                "City Health in Helm = clinics + EMS. Hospital occupancy = provincial extract gap."
            )
        ...
        if "library" in q:
            lines.append("- Libraries are P3 in Helm — brief Health, Water, Safety, Housing first.")
        if risks:
            top = risks[0]
...
_FORMAT_SYSTEM = (
    "You are Helm, the City of Cape Town strategic intelligence OS. "
...
def ask_helm(req: AskRequest, settings: Settings | None = None) -> AskResponse:
    ...
    context_json, citations, facts, gaps = _retrieve_context(req.question, settings.helm_municipality)
